...
const getAllCategories = async (chatId, page = 1, msg_id = null) => {
...
const paginationCategory = async (chatId, action, msg_id = null) => {
  let user = await User.findOne({ chatId }).lean();
  let page = 1;
  ...
  await getAllCategories(chatId, page, msg_id);
};
